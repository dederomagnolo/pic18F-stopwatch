...
unsigned char ReadAddrXLCD(void)
{
        ...
        data = (unsigned)((DATA_PORT<<4)&0xf0);     // Read the nibble into the upper nibble of data
#endif
...
#else                                   // Lower nibble interface
        data |= (unsigned)(DATA_PORT&0x0f);         // Read the nibble into the lower nibble of data
#endif
        E_PIN = 0;
        RW_PIN = 0;                     // Reset the control lines
#endif
        return (unsigned)((data&0x7f));             // Return the address, Mask off the busy bit
}
